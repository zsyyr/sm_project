...
from post.tw_post import TwPost
from comment.tw_comment import TwComment

logger = logging.getLogger('crawler.tw_extractor')
...
class TwExtractor(Extractor):
    def __init__(self, driver): 
        ...
        self.post_list_xpath = '//main//section//div[@data-testid="cellInnerDiv"]'
        self.post_account_xpath = './/div[@data-testid="User-Names"]//span/span'
        self.post_time_xpath = './/time'
        self.post_content_xpath = './/div[@data-testid="tweetText"]'
        self.post_up_n_xpath = './/div[@data-testid="like"]//span'
        self.post_retweet_n_xpath = './/div[@data-testid="retweet"]//span'
        self.post_comment_n_xpath = './/div[@data-testid="reply"]//span'
        ...
        self.post_next_7_xpath = './following-sibling::div[7]' 
        self.post_promoted_xpath = './/div[@id="id__qstjrdiveqq"]'
    ...
    def extract_post_time(self, post_el):
        ...
        time_s = ''
        try:
            time_el = post_el.find_element(By.XPATH, self.post_time_xpath)
            time_s = time_el.get_attribute('datetime')
            logger.debug(f'time string: {time_s}')
    ...
    def extract_post_content(self, post_el):
        try:
            ...
            post_content_str = post_content_el.text
            end = time.perf_counter()
    ...
    def extract_post_account(self, post_el):
        try:            
            start = time.perf_counter()
            post_account_el = post_el.find_element(By.XPATH, self.post_account_xpath)
            end = time.perf_counter()
            logger.debug(f'runing time of extracting post news description: {end - start}')
            return post_account_el.text
        except Exception as e:
    ...
    def extract_post_retweet_number(self, post_el):
        try:
            start = time.perf_counter()
            retweet_num_el =post_el.find_element(By.XPATH, self.post_retweet_n_xpath)
            end = time.perf_counter()
            logger.debug(f'runing time of extracting post up number: {end - start}')
            return retweet_num_el.text
        except Exception as e:
            logger.debug('extracting post up number failed!')  
            return ' '
        
    def extract_post_comment_number(self, post_el):
        try:
            start = time.perf_counter()
            comment_num_el =post_el.find_element(By.XPATH, self.post_comment_n_xpath)
            end = time.perf_counter()
            logger.debug(f'runing time of extracting post up number: {end - start}')
            return comment_num_el.text
        except Exception as e:
            logger.debug('extracting post up number failed!')  
            return ' '
    ...
    def get_post_el_by_waiting_check(self, post_driver, post_el_xpath, scroll_detect_wait_time):
        SCROLL_DETECT_TIME = 3        
        post_el = post_driver
        while SCROLL_DETECT_TIME:
            try:
                post_el = post_el.find_element(By.XPATH, post_el_xpath) 
                ...
                try:
                    test_content = post_el.find_element(By.XPATH, self.post_time_xpath)   # a post's div may be find while it's elements are all unavailable, so must check the post's time element as well.
                except NoSuchElementException:
                    try:
                        if post_el.find_element(By.XPATH, self.post_promoted_xpath):      # skip advis
                            post_el = post_el.find_element(By.XPATH, post_el_xpath)
                    except:
                        post_el = post_el.find_element(By.XPATH, self.post_next_7_xpath)   #skip comment accounts column
                self.post_failed = 0
                time.sleep(3)
                break                
    ...
    def extract_account_posts(self, account, config):
        ...
        while True:
            ...
            try:
                ...
                # print(publish_time_str)
                post.publish_time = datetime_util.parse_tw_time(publish_time_str, datetime.datetime.now())
            ...
            try:
                ...
                logger.exception(e)
            try:
                post.account = self.extract_post_account(self.post_el)
            except Exception as e:
                post.account = ''
                logger.exception(e)
            try:    
                ...
                logger.exception(e)
            try:                
                comment_num = self.extract_post_comment_number(self.post_el)
                post.comment_num = util.parse_number(util.get_retweet_comment_str(comment_num))                
            except Exception as e:
                post.comment_num = 0
                logger.exception(e)
            try:
                retweet_num = self.extract_post_retweet_number(self.post_el)
